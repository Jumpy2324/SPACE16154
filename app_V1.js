...
function featured(){
  ...
  return `<section class="panel featured">
    <div class="section-head">
      <h2>☆ &nbsp; MATCH IN EVIDENZA</h2>
      <span>${f.league} ›</span>
    </div>
    <div class="match-hero">
      <div class="club">${crest(f.homeCode,f.homeClass)}<strong>${f.home}</strong></div>
      <div class="match-time"><b>${f.time}</b><span>OGGI</span><em>VS</em></div>
      <div class="club">${crest(f.awayCode,f.awayClass)}<strong>${f.away}</strong></div>
    </div>
    ...
    <button class="primary-btn" data-open-analysis="featured">
      VAI ALL'ANALISI COMPLETA <span>›</span>
    </button>
  </section>`;
}
...
function upcoming(){
 return `<section class="panel">
   <div class="section-head">
     ...
     <button class="text-btn" data-nav="matches">VEDI TUTTE ›</button>
   </div>
   <div class="fixtures">
   ${DEMO.upcoming.map(x=>`<div class="fixture">
      ...
      <button class="star" aria-label="Preferito">☆</button>
   </div>`).join("")}
   </div>
 </section>`;
}
...
function signals(){
 return `<section class="panel signals">
   <div class="section-head">
     ...
     <button class="text-btn" data-nav="analysis">VEDI TUTTI ›</button>
   </div>
   <div class="signal-row">
   ${DEMO.signals.map(s=>`<article class="signal ${s[4]}">
      <div class="signal-type">${s[0]}</div>
      <div class="signal-main">${s[1]}</div>
      <div class="signal-match">${s[2]}</div>
      <div class="confidence-line">
        CONFIDENCE
        ${info("Confidence","Indica quanto i principali indicatori analizzati convergono verso la previsione. Non rappresenta una seconda probabilità dell'esito.")}
        <strong>${s[3]}</strong>
      </div>
 ...
 <div class="page">
  <div class="page-title-row">
    <div>
      ...
      <h1>DASHBOARD</h1>
      <p>Panoramica generale e segnali principali</p>
    </div>
...
function matches(){
 document.querySelector("#main").innerHTML = `<div class="page">
   <div class="eyebrow">CALENDARIO</div><h1>Match Day</h1>
   <p class="subtitle">Partite filtrate e ordinate per orario</p>
   <div class="filters">
     <button class="filter active">Tutti</button>
     <button class="filter">Premier League</button>
     <button class="filter">Serie A</button>
     <button class="filter">La Liga</button>
     <button class="filter">Bundesliga</button>
     <button class="filter">Ligue 1</button>
   </div>
   <section class="panel">
    <div class="section-head"><h2>PARTITE</h2><span>5 disponibili</span></div>
    ${[
    ...
    ].map(m=>`<button class="match-list" data-open-analysis="match">
      <span>${m[0]}</span>
...
function analysis(){
 document.querySelector("#main").innerHTML = `<div class="page analysis-page">
  ...
  <h1>Milan — Roma</h1>
  <p class="subtitle">Analisi pre-match · dati demo</p>

  <section class="panel result-panel">
    <div class="prediction-grid">
      ${predCard("ESITO","PIÙ PROBABILE","1",68)}
      ${predCard("GOL/NO GOL","PIÙ PROBABILE","GOL",71)}
      ${predCard("OVER/UNDER","PIÙ PROBABILE","OVER",64)}
    </div>
  </section>

  <section class="panel">
    <div class="confidence-head">
      <h2>Confidence ${info("Confidence","Indica quanto i principali indicatori analizzati convergono verso la previsione. Non rappresenta la probabilità dell'esito.")}</h2>
      <strong>76%</strong>
    </div>
    <div class="big-progress"><i style="width:76%"></i></div>
  </section>

  <section class="panel">
    <div class="section-head"><h2>CONFRONTO SQUADRE</h2><span>Milan &nbsp;&nbsp;&nbsp; Roma</span></div>
  ...
  <section class="panel">
    <div class="section-head"><h2>STATISTICHE AVANZATE</h2></div>
    <div class="advanced">
      <div><small>Tiri</small><b>14 — 9</b></div>
      <div><small>Tiri in porta</small><b>6 — 4</b></div>
      <div><small>Possesso</small><b>58% — 42%</b></div>
      <div><small>Corner</small><b>6 — 4</b></div>
      <div><small>Ultimo terzo</small><b>64 — 47</b></div>
      <div><small>Clean sheet</small><b>42% — 31%</b></div>
    </div>
  </section>
 </div>`;
 bindCommon();
...
function row(a,b,c,t){
  return `<div class="compare-row"><span>${a} ${info(a,t)}</span><b>${b}</b><b>${c}</b></div>`;
}
function mini(a,b,t){
  return `<div class="mini-card"><span>${a} ${info(a,t)}</span><b>${b}</b></div>`;
}

function simple(title, text){
 document.querySelector("#main").innerHTML = `
 <div class="page empty">
   <div class="eyebrow">SPACE16154</div><h1>${title}</h1><p>${text}</p>
   <div class="panel demo-note">
     <b>V1 · DATI DEMO</b>
     <span>Questa sezione verrà collegata ai dati reali quando integreremo SportMonks.</span>
   </div>
 </div>`;
 bindCommon();
}
...
function bindCommon(){
 ...
 if(menu) menu.onclick = ()=>showInfo("Menu","Il menu laterale verrà attivato nella prossima evoluzione della V1.");

 const bell = document.querySelector("#bellBtn");
 if(bell) bell.onclick = ()=>showInfo("Notifiche","Qui verranno visualizzati i principali segnali e aggiornamenti.");
}

function showInfo(text){
 const o=document.createElement("div");
 o.className="modal-backdrop";
 o.innerHTML=`<div class="info-modal">
    <button class="modal-x" aria-label="Chiudi">×</button>
    <div class="eyebrow">INFO</div><p>${text}</p>
  </div>`;
