...
function signalColor(kind){
  return kind==="convergence" ? "signal-confidence" : kind==="value" ? "signal-value" : "signal-form";
}
function signalLabel(kind){
  return kind==="convergence" ? "CONFIDENCE" : kind==="value" ? "VALUE" : "FORMA MODELLO";
}
function signalCard(s){
  const extra=s.edge?`<span class="signal-extra signal-value">EDGE ${s.edge}</span>`:s.form?`<span class="signal-extra signal-form">FORMA ${s.form}</span>`:"";
  const infoText=s.kind==="convergence"
    ?"Confidence / Alta Convergenza: misura quanto più famiglie di indicatori importanti puntano nella stessa direzione."
    :s.kind==="value"
    ?"Value / Edge: confronta la stima del modello con la probabilità implicita del mercato. È dinamico perché la quota può cambiare nel tempo."
    :"Forma Modello: segnala una condizione di forma recente particolarmente forte secondo il modello.";
  return `<article class="signal ${s.kind}" data-signal-kind="${s.kind}">
    <div class="signal-type ${signalColor(s.kind)}">${s.type} ${info(s.type,infoText)}</div>
    <div class="signal-outcome most-probable-result">${s.outcome}</div>
...
function signals(){
  return `<section class="panel signals top-signals-panel">
    <div class="section-head"><h2>◎ &nbsp; TOP SIGNALS</h2><span>SEGNALI RILEVANTI</span></div>
    <div class="top-signal-filters" role="tablist">
      <button class="signal-filter active" data-filter="all">Tutti</button>
      <button class="signal-filter confidence-filter" data-filter="convergence">Confidence</button>
      <button class="signal-filter value-filter" data-filter="value">Value</button>
      <button class="signal-filter forma-filter" data-filter="form">Forma Modello</button>
    </div>
    <div class="signal-row">${DEMO.signals.map(signalCard).join("")}</div>
    <div class="signal-legend">
      <span class="legend-convergence">● Confidence / Alta Convergenza</span>
      <span class="legend-value">● Value</span>
      <span class="legend-form">● Forma Modello</span>
    </div>
  </section>`;
}

function bindTopSignalFilters(){
  document.querySelectorAll(".signal-filter").forEach(btn=>{
    btn.onclick=()=>{
      ...
      document.querySelectorAll("[data-signal-kind]").forEach(card=>{
        card.style.display=(filter==="all" || card.dataset.signalKind===filter) ? "" : "none";
      });
    };
  });
}
...
function controlRow(a,b,c){
  const note=a==="Pressione alta"
    ? info("Pressione alta","Misura quanto spesso e quanto efficacemente la squadra recupera palla con pressione alta, soprattutto nell’ultimo terzo. Tiene conto di dove avviene il recupero e del contesto dell’avversario.")
    : a==="PPDA"
    ? info("PPDA","Indicatore della pressione esercitata: il modello lo combina con recuperi alti, altezza difensiva ed efficacia del pressing. Il valore viene reinterpretato nel contesto dell’avversario.")
    : "";
  return `<div class="control-row"><span>${a}${note}</span><b>${b}</b><b>${c}</b></div>`;
}
...
function analysis(){
  ...
  document.querySelector("#main").innerHTML=`<div class="page analysis-page">
    ...
    <section class="panel">
      ...
      ${DEMO.comparison.map(r=>`<div class="compare-row"><span>${r[0]}${["xG","xGA","ULTIMO TERZO","QUALITÀ TIRO","QUALITÀ TIRO (xG per tiro)"].includes(r[0]) ? info(r[0],"Parametro tecnico utilizzato dal modello insieme agli altri indicatori della stessa macroarea.") : ""}</span><b>${r[1]}</b><b>${r[2]}</b><em>${r[4]}</em></div>`).join("")}
    ...
    <section class="panel model-performance"><div class="section-head"><h2>MODEL PERFORMANCE</h2><span>STORICO</span></div>
      <div class="performance-grid">
        <div><span>Pronostici analizzati</span><b>247</b></div>
        <div><span>Accuratezza 1X2</span><b>62,3%</b></div>
        <div><span>Confidence media</span><b class="green-text">78</b></div>
        <div><span>High Confidence</span><b class="green-text">71,1%</b></div>
      </div>
      <p class="performance-note">Dati demo: lo storico reale verrà popolato dalle previsioni salvate prima dell'evento e confrontato automaticamente con i risultati reali.</p>
    </section>
