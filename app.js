  ...
  return `<section class="panel probabilities">
    ...
    <div class="prob-grid">
      <div class="market-box"><small>1X2</small><div class="one-x-two">
        ${f.oneXtwo.map(x=>`<div><span>${x[0]}</span><b>${x[1]}</b><em>${x[2]}</em></div>`).join("")}
      </div></div>
...
function signalCard(s){
  ...
  return `<article class="signal ${s.kind}">
    ...
    <div class="signal-meta"><span>CONFIDENCE ${s.confidence}</span>${extra}</div>
...
function analysis(){
  ...
  document.querySelector("#main").innerHTML=`<div class="page analysis-page">
    ...
    <section class="panel">
      ...
      ${DEMO.comparison.slice(0,4).map(r=>`<div class="compare-row"><span>${r[0]}</span><b>${r[1]}</b><b>${r[2]}</b><em>${r[4]}</em></div>`).join("")}
    </section>

    <section class="panel form-panel"><div class="section-head"><h2>FORMA ${info("Forma","Indice composito delle ultime 5 partite, ponderato per qualità della performance e recenza.")}</h2><span>ULTIME 5</span></div>
      <div class="form-team"><div><strong>Milan</strong><div class="dots">${formDots(f.milan[1])}</div></div><b class="form-score green-text">${f.milan[2]} ${f.milan[0]}</b></div>
      <div class="form-team"><div><strong>Roma</strong><div class="dots">${formDots(f.roma[1])}</div></div><b class="form-score neutral-text">${f.roma[2]} ${f.roma[0]}</b></div>
    </section>
    ...
    <section class="panel technical">
      <div class="section-head"><h2>DATI TECNICI</h2><span>DETTAGLIO</span></div>
      <div class="technical-grid">
        ${DEMO.comparison.slice(4).map(r=>`<div class="technical-card"><span>${r[0]}</span><b>${r[1]} — ${r[2]}</b><small>${r[3]}</small></div>`).join("")}
        <div class="technical-card"><span>Possesso</span><b>58% — 42%</b><small>Contesto, con peso ridotto rispetto alla pericolosità.</small></div>
        <div class="technical-card"><span>Corner medi</span><b>6 — 4</b><small>Segnale secondario di pressione.</small></div>
      </div>
    </section>
    ...
    <section class="panel">
      ...
      <div class="opponent"><div><span>Milan</span><b>72</b></div><div><span>Roma</span><b>68</b></div></div>
    </section>
...
function bind(){
  ...
  const m=document.querySelector("#menuBtn");if(m)m.onclick=()=>showInfo("La navigazione principale resta nella barra inferiore.");
}
function route(n){
