...
function dashboard(){
 ...
 const home=MODEL.teams.find(t=>t.name===m.home)||MODEL.teams[0];
 const away=MODEL.teams.find(t=>t.name===m.away)||MODEL.teams[1];
 const h=highest(m.p);
 const macroRows=MACROS.map(([k,l])=>{
   const a=home.m[k], b=away.m[k], d=a-b;
   return `<div class="dash-macro-row">
     <div class="dash-macro-name"><span>${k==="threat"?"⚡":k==="defense"?"⬡":k==="form"?"↗":k==="opponent"?"◎":k==="pressing"?"♧":"♙"}</span><div><b>${l}</b><small>${INFO[k][1]}</small></div></div>
     <strong class="${a>=b?"green":""}">${a}</strong><i class="dash-meter ${a>=b?"good":"warn"}"></i>
     <em class="${d>=0?"green":"red"}">${d>=0?"+":""}${d}</em>
     <strong class="${b>a?"green":""}">${b}</strong><i class="dash-meter ${b>a?"good":"warn"}"></i>
     <button class="dash-chevron" onclick="infoMacro('${k}')">⌄</button>
   </div>`;
 }).join("");
 const signals=MODEL.matches.map((x,i)=>{
   const hh=highest(x.p), kind=i===1?"Value":i===2?"Forma":"Confidence";
   const klass=kind.toLowerCase();
   const score=kind==="Value"?x.value:kind==="Forma"?x.form:x.confidence;
   return `<button class="dash-signal ${klass}" onclick="location.hash='match/${x.id}'">
     <div class="dash-signal-icon">${kind==="Confidence"?"★":kind==="Value"?"◆":"↗"}</div>
     <div class="dash-signal-copy"><small>${kind==="Confidence"?"HIGH CONFIDENCE":kind==="Value"?"VALUE BET":"FORMA"}</small><b>${esc(x.home)} - ${esc(x.away)}</b><span>${kind==="Value"?"Doppia chance: X2":kind==="Forma"?"Over 2.5":"Esito: "+hh.a[0]}</span></div>
     <div class="dash-signal-stat"><small>Probabilità</small><b>${hh.a[1]}%</b></div>
     <div class="dash-signal-stat"><small>${kind}</small><b>${kind==="Confidence"?Math.round(score/10)+"/10":kind==="Value"?Math.round(score/2)+"/10":Math.round(score/10)+"/10"}</b></div>
   </button>`;
 }).join("");
 const tech=m.technical.slice(0,8).map(r=>`<div class="dash-tech-cell"><small>${esc(r[0])}</small><b class="green">${r[1]}</b><span>${r[2]}</span></div>`).join("");
 $("#app").innerHTML=`<section class="dash-pixel-page">
   <div class="dash-pixel-grid">
     <div class="dash-left">
       <section class="dash-panel dash-featured">
         <div class="dash-panel-title"><span>MATCH IN EVIDENZA</span></div>
         <div class="dash-featured-inner">
           <div class="dash-club"><span class="dash-crest">MIL</span><b>MILAN</b></div>
           <div class="dash-vs"><small>${esc(m.league)} · ${esc(m.date)}</small><strong>VS</strong><button onclick="location.hash='match/${m.id}'">⌁ Analisi completa</button></div>
           <div class="dash-club"><span class="dash-crest">ROM</span><b>ROMA</b></div>
         </div>
         <div class="dash-subtitle">PROBABILITÀ DEL MODELLO <button class="info" onclick="showInfo('threat')">i</button></div>
         <div class="dash-prob-grid">
           <div class="dash-prob-box"><b>1X2</b><div><span class="best">1<strong>${m.p.one}%</strong></span><span>X<strong>${m.p.x}%</strong></span><span>2<strong>${m.p.two}%</strong></span></div><small>Risultato più probabile: <em>${h.a[0]} (${h.a[1]}%)</em></small></div>
           <div class="dash-prob-box"><b>UNDER / OVER 2.5</b><div class="two"><span class="${m.p.under>=m.p.over?"best":""}">Under 2.5<strong>${m.p.under}%</strong></span><span class="${m.p.over>m.p.under?"best":""}">Over 2.5<strong>${m.p.over}%</strong></span></div><small>Soglia: 2.5 gol</small></div>
           <div class="dash-prob-box"><b>GOL / NO GOL</b><div class="two"><span class="${m.p.gg>=m.p.ng?"best":""}">Gol<strong>${m.p.gg}%</strong></span><span class="${m.p.ng>m.p.gg?"best":""}">No Gol<strong>${m.p.ng}%</strong></span></div><small>Probabilità di Gol: <em>${m.p.gg>=m.p.ng?"Alta":"Bassa"}</em></small></div>
         </div>
       </section>

       <section class="dash-panel">
         <div class="dash-panel-title dash-title-row"><span>TOP SIGNALS</span><div><span>Confidence</span><span>Value</span><span>Forma</span></div></div>
         <div class="dash-signals">${signals}</div>
         <button class="dash-see-all" onclick="location.hash='signals'">Vedi tutti i segnali</button>
       </section>
     </div>

     <div class="dash-right">
       <section class="dash-panel">
         <div class="dash-panel-title">ANALISI DEL MODELLO <button class="info" onclick="showInfo('threat')">i</button></div>
         <div class="dash-score-head">
           <div><span class="dash-crest sm">MIL</span><b>MILAN</b><strong class="green">${home.score}</strong><small>/100</small></div>
           <div class="dash-model-score"><small>MODEL SCORE</small><strong>+${home.score-away.score}</strong><em>Vantaggio MILAN</em></div>
           <div><strong>${away.score}</strong><small>/100</small><b>ROMA</b><span class="dash-crest sm">ROM</span></div>
         </div>
         <div class="dash-macros">${macroRows}</div>
         <div class="dash-note">ⓘ I valori vanno da 0 a 100. Più è alto, migliore è la performance dell'area.</div>
       </section>

       <section class="dash-panel">
         <div class="dash-panel-title dash-title-row"><span>DATI TECNICI - CONFRONTO</span><button onclick="location.hash='match/${m.id}'">Vedi tutti</button></div>
         <div class="dash-tech-grid">${tech}</div>
       </section>
     </div>
   </div>
 </section>`;
