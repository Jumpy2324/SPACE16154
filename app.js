const DEMO={
  ...
  games:[
    ["18:00","Milan","Roma","74%","MIL","ROM","milan","roma","Serie A",true],
  ...
  comparison:[
    ...
    ["ULTIME 5 PARTITE","78%","61%","Indice recente ponderato per qualità e recenza.","+17%"],
...
const featuredGame=()=>DEMO.games.find(g=>g[9])||DEMO.games[0];
...
function featured(){
  ...
  const f={league:g[8],home:g[1],away:g[2],time:g[0],hc:g[4],ac:g[5],hcl:g[6],acl:g[7],oneXtwo:DEMO.featured.oneXtwo};
  return `<section class="panel">
    ...
    <div class="prediction-grid">
      ${f.oneXtwo.map((x,i)=>`<div class="pred-card ${i===f.oneXtwo.reduce((best,v,idx)=>parseFloat(v[1])>parseFloat(f.oneXtwo[best][1])?idx:best,0)?"primary":""}""><small>${i===0?"1X2":""}</small><strong>${x[0]}</strong><em>${x[1]}</em><span>${x[2]}</span></div>`).join("")}
    </div>
...
function signalCard(s){
  ...
  const infoText=s.kind==="convergence"
    ?"Alta Convergenza: più famiglie di indicatori importanti puntano nella stessa direzione. È una misura di coerenza del segnale, non una seconda probabilità dell'esito."
    :s.kind==="value"
    ...
    :"Forma Modello: segnala una squadra con rendimento recente particolarmente forte secondo l'indice proprietario.";
  const label=s.kind==="convergence"?"Alta Convergenza":s.kind==="value"?"Value":"Forma Modello";
  return `<article class="signal ${s.kind}">
    <div class="signal-type">${label} ${info(label,infoText)}</div>
    <div class="signal-outcome signal-most-probable">${s.outcome}</div>
...
function signals(){
  return `<section class="panel signals"><div class="section-head"><h2>◎ &nbsp; TOP SIGNALS</h2></div>
    <div class="signal-filters" role="tablist">
      <button class="signal-filter active" data-signal-filter="all">Tutti</button>
      <button class="signal-filter confidence-filter" data-signal-filter="convergence">Confidence</button>
      <button class="signal-filter value-filter" data-signal-filter="value">Value</button>
      <button class="signal-filter form-filter" data-signal-filter="form">Forma Modello</button>
    </div>
...
const METRIC_INFO = {
  "xG":"Expected Goals: stima della qualità delle occasioni create, pesata sulla pericolosità di ogni conclusione.",
  "xGA":"Expected Goals Against: stima della qualità delle occasioni concesse all'avversario.",
  "Ultimo terzo":"Misura presenza, progressione e capacità di creare pericolo nell'ultimo terzo di campo.",
  "QUALITÀ TIRO (xG per tiro)":"Valuta quanto sono pericolose mediamente le conclusioni, evitando di premiare soltanto il volume dei tiri.",
  "Pressione alta":"Misura quanto spesso e quanto efficacemente la squadra recupera palla con pressione alta, considerando anche dove avviene il recupero e il contesto dell'avversario.",
  "PPDA":"Passes Per Defensive Action: misura quante azioni/passaggi l'avversario può effettuare prima di subire un'azione difensiva. Il modello lo rielabora insieme a recuperi alti e contesto avversario."
};
function metricLabel(label){
  const key=label==="QUALITÀ TIRO (xG per tiro)"?"QUALITÀ TIRO (xG per tiro)":label;
  return METRIC_INFO[key] ? `${label} ${info(label,METRIC_INFO[key])}` : label;
}
function controlRow(a,b,c){
  return `<div class="control-row"><span>${metricLabel(a)}</span><b>${b}</b><b>${c}</b></div>`;
}
...
function analysis(){
  ...
  document.querySelector("#main").innerHTML=`<div class="page analysis-page">
    ...
    <section class="panel">
      ...
      ${DEMO.comparison.map(r=>`<div class="compare-row"><span>${metricLabel(r[0])}</span><b>${r[1]}</b><b>${r[2]}</b><em>${r[4]}</em></div>`).join("")}
    ...
    <section class="panel form-panel"><div class="section-head"><h2>ULTIME 5 PARTITE ${info("Ultime 5 partite","La forma confronta le ultime cinque partite con il rendimento precedente della squadra. Tiene conto dei risultati e delle prestazioni offensive e difensive, considerando anche la qualità delle occasioni e il modo in cui vengono create e concesse.")}</h2><span>RECENTE</span></div>
      ...
      <div class="form-team"><div><strong>Roma</strong><div class="dots">${formDots(f.roma[1])}</div></div><b class="form-score v1-white-marker">${f.roma[2]}</b></div>
    </section>
    ...
    <section class="panel v1-model-performance">
      <div class="section-head"><h2>MODEL PERFORMANCE</h2><span>STORICO</span></div>
      <div class="v1-kpis">
        <div class="v1-kpi"><small>PRONOSTICI ANALIZZATI</small><b>247</b></div>
        <div class="v1-kpi"><small>ACCURATEZZA 1X2</small><b>62.3%</b></div>
        <div class="v1-kpi"><small>CONFIDENCE MEDIA</small><b class="v1-confidence">78</b></div>
        <div class="v1-kpi"><small>HIGH CONFIDENCE</small><b>71.1%</b></div>
      </div>
      <p class="compare-footnote">Dati demo: lo storico reale verrà popolato dalle previsioni salvate prima dell'evento e confrontato automaticamente con il risultato reale.</p>
    </section>
...
function bind(){
  ...
  document.querySelectorAll("[data-signal-filter]").forEach(b=>b.onclick=()=>{
    const filter=b.dataset.signalFilter;
    document.querySelectorAll("[data-signal-filter]").forEach(x=>x.classList.toggle("active",x===b));
...
/* ===== V1 FINAL UI CONFIG =====
   Presentation contract for the agreed frontend changes.
   Keep business/model calculations in the existing backend layer.
*/
const V1_FINAL_UI_CONFIG = Object.freeze({
  colors: Object.freeze({
    confidence: "green",
    value: "orange",
    formaModello: "blue",
    mostProbable: "green",
    lowestOpponentStrength: "red",
    highestOpponentStrength: "green",
    neutral: "white"
  }),
  infoMetrics: Object.freeze([
    "xG", "xGA", "Ultimo terzo", "Qualità del tiro", "Pressione alta", "PPDA"
  ]),
  topSignalFilters: Object.freeze(["Tutti", "Confidence", "Value", "Forma Modello"]),
  sections: Object.freeze({
    form: "Ultime 5 partite",
    performance: "Model Performance"
  }),
  featuredMatch: Object.freeze({
    label: "Match in evidenza",
    scope: "big match of the day across selected leagues"
  })
});
