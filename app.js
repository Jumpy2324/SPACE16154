...
const CONFIG={
  version:"4.1.0-functional-pre0302",
...
function shell(a="dashboard"){
 return `<header class="topbar"><div class="top-spacer"></div><div class="brand"><div class="brand-name">SPACE16154</div><div class="brand-sub">FOOTBALL INTELLIGENCE</div></div><div class="engine"><i></i></div></header>
 <main id="main"></main>
...
function dashboard(){
 const all=MATCHES.flatMap(m=>m.signals);
 const conf=all.filter(s=>s.type==="Confidence");
 document.querySelector("#main").innerHTML=`<div class="page"><div class="page-title-row"><div><div class="eyebrow">PANORAMICA</div><h1>DASHBOARD</h1><p>Il quadro sintetico prima di entrare nel dettaglio.</p></div><button class="date-btn">OGGI⌄</button></div>
 <div class="stats">${stat("PARTITE OGGI",MATCHES.length,"Vai alla lista completa","□",70,"matches")}${stat("SEGNALI",all.length,"Opportunità rilevate","◎",Math.min(100,all.length*12),"signals")}${stat("CONFIDENCE MEDIA",conf.length?R(avg(conf.map(s=>s.score)))+"%":"—","Coerenza dei segnali","◉",conf.length?R(avg(conf.map(s=>s.score))):0,"signals")}${stat("DATA QUALITY","96%","Copertura demo","✓",96,"more")}</div>
 ${featured()}${upcoming()}
 <section class="panel"><div class="section-head"><h2>◎ &nbsp; TOP SIGNALS</h2><button class="text-btn" data-nav="signals">VEDI TUTTI ›</button></div><div class="signal-row">${MATCHES.flatMap(m=>m.signals.map(s=>[s,m])).slice(0,3).map(([s,m])=>signalCard(s,m)).join("")||`<div class="empty-note">Nessun segnale sopra soglia.</div>`}</div></section></div>`;
 bind();
}
function matches(){
...
function renderLeague(l){
 ...
 <section class="panel"><div class="section-head"><h2>TREND MODEL SCORE</h2><span>ULTIMI PERIODI</span></div><div class="trend-grid2">${arr.slice(0,2).map(t=>`<div class="trend-card2"><div><b>${t.name}</b><strong>${t.score}</strong></div><div class="spark2">${t.trend.map(v=>`<i style="height:${v}%"></i>`).join("")}</div><small>${t.trend.at(-1)>=t.trend[0]?"Trend positivo":"Trend in calo"}</small></div>`).join("")}</div></section>`;
}
function teamProfile(id){
 ...
 <section class="panel"><div class="section-head"><h2>STATISTICHE TECNICHE</h2><span>SNELLITE</span></div><div class="technical-grid">${[["xG",t.raw.xg.toFixed(2)],["xGA",t.raw.xga.toFixed(2)],["Ultimo terzo",R(t.raw.third)],["Qualità tiro",t.raw.shotq.toFixed(2)],["PPDA",t.raw.ppda.toFixed(1)]].map(x=>`<div><small>${x[0]}</small><b>${x[1]}</b></div>`).join("")}</div></section>
...
function matchDetail(id){
 ...
 <section class="panel"><div class="section-head"><h2>DATI TECNICI — CONFRONTO</h2><span>SNELLITI</span></div><div class="compare-head"><span>INDICATORE</span><b>${m.h.code}</b><b>${m.a.code}</b></div>${technical(m).map(r=>`<div class="compare-row"><span>${r[0]} ${info(r[0],TECH_INFO[r[0]])}</span><b>${r[1]}</b><b>${r[2]}</b></div>`).join("")}</section>
...
function renderSig(){
 const a=allSignals().filter(x=>(SIG.type==="Tutti"||x.type===SIG.type)&&(SIG.league==="Tutti"||x.m.league===SIG.league));
 document.querySelector("#sigCount").textContent=a.length+" trovati";
 document.querySelector("#sigList").innerHTML=a.length?a.map(x=>`<button class="signal-detail ${x.kind}" data-match="${x.m.id}"><div><span>${signalLabel(x)}</span><b>${x.m.h.name} — ${x.m.a.name}</b><small>${x.m.league} · ${x.pick} · ${x.prob}%</small><p>${x.reason}</p></div><strong>${x.type==="Value"?"+"+x.score+"%":x.type==="Forma"?"+"+x.score:x.score+"%"}</strong></button>`).join(""):`<div class="empty-note">Nessun segnale rilevante con questi filtri.</div>`;
}
...
route("dashboard");
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=4.1.0-functional-pre0302",{updateViaCache:"none"}).catch(()=>{}));
